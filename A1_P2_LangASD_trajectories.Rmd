...
ageModel=lm(Age~Diagnosis,data)
summary(ageModel)
...
```{r}
#Choosing ID as a random effect as there is unsystematic variance between the children
model1=lmerTest::lmer(CHI_MLU~1+Visit+Diagnosis+(1+Visit|ID),data,REML=F)
#Plot tells us that our model is biased --> violating lineraity --> this is where you can add a geom_smooth to a line plot --> growth-curve model --> see further down)
summary(model1)
```
...
```{r}
#Look at Chi-squared and R-squared 
#Creating a NullModel with Visit as only predictor (to depict a natural development over time)
nullModel=lmerTest::lmer(CHI_MLU~1+Visit+(1+Visit|ID),data, REML=F)
summary(nullModel)
