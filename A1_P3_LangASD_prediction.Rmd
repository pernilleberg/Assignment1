...
#Loading libraries
library(pacman)
p_load(modelr,lmerTest,ggplot2,dplyr,tidyverse,MuMIn,pacman,stringr,tidyverse,plyr,caret,Metrics,groupdata2)
...
```
An alright performance (rmse = average error, 0.35 is okay)
...
```
So our model was okay at explaning data, but horrible at explaning new data (average error of 0.72, when you want to diagnose something is bad)
...
THE EXTENDED MODEL IS RELATIVELY GOOD AT EXPLANING THE ORIGINAL DATA (TRAINING_DATA) BUT FAILS AT PREDICTING/GENERALIZE TO NEW DATA (TEST_DATA)
...
#Ca det samme som loopet --> betyder at vi kan få samme info fra store dataset som det lille dataset
...
return(data.frame(perf_test = perf_test_list, perf_train = perf_train_list, model = model_string))
}
...
bernie_frame=subset(test_data, ID==202)
summary(model1)
